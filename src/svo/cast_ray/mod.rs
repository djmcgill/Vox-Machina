use nalgebra::{Vector3, Norm, Iterable};
use svo::*;
...
impl SVO {
    ...
    fn cast_ray_sanitised(&self, ray_origin: Vector3<f32>, ray_dir: Vector3<f32>, inv_ray_dir: Vector3<f32>) -> Option<Vector3<f32>> {
        let min_dist = 0.;
        let max_dist = 100000.;
        ...
        match *self {
            ...
            SVO::Octants(ref octants) => {
                ...
                let test_child = |(above_x, above_y, above_z): (bool, bool, bool)| -> Option<Vector3<f32>> {
                    ...
                    octants[child_ix].cast_ray_sanitised(new_origin, ray_dir, inv_ray_dir).map (|child_hit: Vector3<f32>| {
